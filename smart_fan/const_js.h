       ...
       const form = document.getElementById('config-form');
             form.addEventListener('submit', (event) => {
                  ...
                  const xhr = new XMLHttpRequest();
                   ...
                   var on_hum = document.getElementById("on_hum").value;
                   var off_hum = document.getElementById("off_hum").value;
                      if (off_hum >= on_hum) {
                               document.getElementById('response').innerHTML = 'Значение на выключение не должно равняться <br>или быть больще, чем значение на включение!';
                               return false;
                             }else{
                               xhr.send(formData);
                              }
